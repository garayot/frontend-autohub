import {
  ...
  async function getDatas(url = "", keyword = "") {
    ...
    const response = await fetch(backendURL + "/api/orders" + queryParams, {
      headers: {
      ...
      try {
        ...
        json.data.forEach((order) => {
          ...
          <tr data-id="${order.order_ID}">
            ...
            <td>${order.units.make} ${order.units.model_name} ${order.units.model_year}</td>  
            <td>${order.date}</td>
            <td>${order.status}</td>
            <td>${order.units.price}</td>
         </tr>         
                        `;
        });
  
        // Use the container to display the fetch data
        document.getElementById("get_data").innerHTML = rows;
